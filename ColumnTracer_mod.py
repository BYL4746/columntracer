# -*- coding: utf-8 -*-
"""
Created on Thu Apr  1 20:29:59 2021
...
class Column_tracer():
    ...
    def characteristic_equation(self):
        ...
        self.ax1.set_xlim(0, np.pi * 10)
        self.ax1.axhline(y=0, c = 'k', lw = 1)
        self.ax1.set_xlabel(r'$\beta$', weight = 'bold', fontsize = 14)
        self.ax1.set_ylabel(r'$F(Pe, \beta)=\beta$ $\cot$ $\beta - \frac{\beta}{Pe} + \frac{Pe}{4}$', weight = 'bold', fontsize = 14)
        self.ax1.set_yticklabels([])
        self.ax1.set_yticks([])
        self.ax1.set_xticks(singularities)
        self.ax1.set_xticklabels(['{}$\pi$'.format(i) for i in range(len(singularities))])
        self.ax1.set_title('Characteristic Equation for Eigenvalues', weight = 'bold', fontsize = 14)
        ...
        for i in range(len(singularities)-1):
            ...
            self.ax1.plot(xs,ys, c = 'r', lw = 1.5)
            self.ax1.axvline(x=s2, c = 'k', ls = '--', lw = 1)
        
        # add an annotation to point out the first few betas
        arrowprops={'arrowstyle':'-|>','connectionstyle':'arc3,rad=-.4','fc':'k'}
        self.ax1.annotate(r'$\beta_{1}$', xy = [1.54,0], xytext = [1,3], fontsize = 12, arrowprops = arrowprops)
        self.ax1.annotate(r'$\beta_{2}$', xy = [3.88,0], xytext = [4.3,3], fontsize = 12, arrowprops = arrowprops)
        self.ax1.annotate(r'$\beta_{3}$', xy = [6.72,0], xytext = [7.5,3], fontsize = 12, arrowprops = arrowprops)
    ...
    def concentration_profile(self):
        ...
        for t in times:
            ...
            # Plot the results
        # fig, ax = plt.subplots()
        self.ax2.set_xlabel('Position in column (cm)', size = 12, weight = 'bold')
        self.ax2.set_ylabel('Concentration (mg/L)', size = 12, weight = 'bold')
        self.ax2.set_title('Column Concentration Profiles', size = 14, weight = 'bold')
        for t,C in zip(times,Cs):
            self.ax2.plot(positions,C, label = 't = {:.1f} hours'.format(t))            
        leg = self.ax2.legend(bbox_to_anchor = (1.02, 0.5), loc = 6, fontsize = 12)
    ...
    def effluent_concentration(self):
        ...
        # Plot the results
        # fig, ax = plt.subplots()
        self.ax3.set_xlabel('Time (hr)', size = 12, weight = 'bold')
        self.ax3.set_ylabel('Concentration (mg/L)', size = 12, weight = 'bold')
        self.ax3.set_title('Column Breakthrough Curve', size = 14, weight = 'bold')
        self.ax3.plot(times, Cs, ls = '-', c = 'r', label = 'Breakthrough curve')
        
        # Add a couple of other lines for explanation of behavior
        xs = [0, self.L/self.U, self.L/self.U, 12]
        ys = [0, 0, self.C0, self.C0]
        self.ax3.plot(xs, ys, ls = '-.', lw = 1, c = 'b', label = 'Plug flow')
        self.ax3.text(0.5,65,'Effects of Dispersion', fontsize = 12)
        arrowprops = {'arrowstyle':'<|-|>'}
        self.ax3.annotate('', xy = (5.5,61), xytext = (0.5,61), ha = 'right', va = 'center', arrowprops = arrowprops)
        leg = self.ax3.legend()
        plt.show()
